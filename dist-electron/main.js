...
const require$$4__namespace = /* @__PURE__ */ _interopNamespaceDefault(require$$4);
class Database {
  static async initialize() {
    const dbPath = require$$4__namespace.join(electron.app.getPath("userData"), "userdata.db");
...
function requireMacos() {
  ...
  macos.exports = async (options) => {
    const { stdout } = await execFile(bin2, getArguments(options));
    return parseMac(stdout);
  };
  macos.exports.sync = (options) => {
    const stdout = childProcess.execFileSync(bin2, getArguments(options), { encoding: "utf8" });
    return parseMac(stdout);
  };
  macos.exports.getOpenWindows = async (options) => {
    const { stdout } = await execFile(bin2, [...getArguments(options), "--open-windows-list"]);
    return parseMac(stdout);
  };
  macos.exports.getOpenWindowsSync = (options) => {
    const stdout = childProcess.execFileSync(bin2, [...getArguments(options), "--open-windows-list"], { encoding: "utf8" });
    return parseMac(stdout);
  };
  return macos.exports;
}
var linux = { exports: {} };
var hasRequiredLinux;
function requireLinux() {
  if (hasRequiredLinux) return linux.exports;
  hasRequiredLinux = 1;
  const { promisify } = require$$0;
  const fs = require$$1;
  const childProcess = require$$2;
  ...
  const getMemoryUsageByPidSync = (pid) => {
    const statm = require$$1.readFileSync(`/proc/${pid}/statm`, "utf8");
    return Number.parseInt(statm.split(" ")[1], 10) * 4096;
  };
  ...
  linux.exports.getOpenWindows = async () => {
    try {
  ...
  linux.exports.getOpenWindowsSync = () => {
    try {
...
function requireNopt() {
  ...
  (function(module2, exports) {
    ...
    var url = require$$0$1, path = require$$4, Stream = require$$0$2.Stream, abbrev2 = requireAbbrev(), os = require$$4$1;
...
function requirePreBinding() {
  ...
  (function(module2, exports) {
    ...
    const napi2 = requireNapi();
    const existsSync = require$$1.existsSync || require$$4.existsSync;
    const path = require$$4;
    module2.exports = exports;
...
function requireNodePreGyp() {
  ...
  (function(module2, exports) {
    module2.exports = exports;
    exports.mockS3Http = requireS3_setup().get_mockS3Http();
    ...
    const inherits2 = require$$0.inherits;
    const cli_commands = [
      "clean",
      "install",
      "reinstall",
...
var windowsBinding;
var hasRequiredWindowsBinding;
function requireWindowsBinding() {
  if (hasRequiredWindowsBinding) return windowsBinding;
  hasRequiredWindowsBinding = 1;
  const binary = requireNodePreGyp();
  const path = require$$4;
  const fs = require$$1;
  const bindingPath = binary.find(path.resolve(path.join(__dirname, "../package.json")));
  const binding = fs.existsSync(bindingPath) ? commonjsRequire(bindingPath) : {
    getActiveWindow: () => {
    },
    getOpenWindows: () => {
    }
...
function requireWindows() {
  ...
  const addon = requireWindowsBinding();
  windows.exports = async () => addon.getActiveWindow();
  windows.exports.getOpenWindows = async () => addon.getOpenWindows();
  windows.exports.sync = addon.getActiveWindow;
  windows.exports.getOpenWindowsSync = addon.getOpenWindows;
  return windows.exports;
}
activeWin.exports = (options) => {
  if (process.platform === "darwin") {
    return requireMacos()(options);
  }
  if (process.platform === "linux") {
    return requireLinux()(options);
  }
  if (process.platform === "win32") {
    return requireWindows()(options);
  }
  return Promise.reject(new Error("macOS, Linux, and Windows only"));
};
activeWin.exports.sync = (options) => {
  if (process.platform === "darwin") {
    return requireMacos().sync(options);
  }
  if (process.platform === "linux") {
    return requireLinux().sync(options);
  }
  if (process.platform === "win32") {
    return requireWindows().sync(options);
  }
  throw new Error("macOS, Linux, and Windows only");
};
activeWin.exports.getOpenWindows = (options) => {
  if (process.platform === "darwin") {
    return requireMacos().getOpenWindows(options);
  }
  if (process.platform === "linux") {
    return requireLinux().getOpenWindows(options);
  }
  if (process.platform === "win32") {
    return requireWindows().getOpenWindows(options);
  }
  return Promise.reject(new Error("macOS, Linux, and Windows only"));
};
activeWin.exports.getOpenWindowsSync = (options) => {
  if (process.platform === "darwin") {
    return requireMacos().getOpenWindowsSync(options);
  }
  if (process.platform === "linux") {
    return requireLinux().getOpenWindowsSync(options);
  }
  if (process.platform === "win32") {
    return requireWindows().getOpenWindowsSync(options);
  }
  throw new Error("macOS, Linux, and Windows only");
};
var activeWinExports = activeWin.exports;
class AppCollector {
  constructor() {
    __publicField(this, "currentApp", null);
    __publicField(this, "collectionInterval", null);
  }
  start() {
    this.collectionInterval = setInterval(async () => {
      var _a;
      try {
        const activeWindow = await activeWinExports.activeWin();
        if (!activeWindow) return;
        const { owner: appName, title } = activeWindow;
        if (this.currentApp && (this.currentApp.name !== appName || ((_a = this.currentApp) == null ? void 0 : _a.title) !== title)) {
        ...
        console.error("Error collecting app usage:", error2);
      }
    }, 1e3);
  }
  async saveCurrentAppUsage() {
...
electron.ipcMain.handle("get-daily-stats", async () => {
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  return await Database.all(
    `SELECT app_name, SUM(duration) as total_duration FROM app_usage WHERE date(start_time) = ? GROUP BY app_name`,
    [today]
  );
});
electron.ipcMain.handle("get-app-usage", async () => {
  return await Database.all(
...
async function createWindow() {
  ...
  const mainWindow = new electron.BrowserWindow({
    ...
    webPreferences: {
      ...
      // 建议设为 true 以提高安全性
      preload: require$$4__namespace.join(__dirname, "preload.js")
      // 添加预加载脚本
    }
  });
  appCollector.start();
  if (process.env.NODE_ENV !== "production") {
    ...
    mainWindow.loadFile(require$$4__namespace.join(__dirname, "../dist/index.html"));
  }
}
